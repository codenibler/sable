...
The next logical milestones are Solana token indexing, historical market-value reconstruction before the first local snapshot, operating-system keyring storage, background refresh, Linux packaging, and then Tauri's iOS target with an explicit synchronization design.
